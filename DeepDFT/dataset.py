...
class AseNeigborListWrapper:
    ...
    def __init__(self, cutoff, atoms):
        self.neighborlist = NewPrimitiveNeighborList(
...
class PymatgenNeighborListWrapper:
    def __init__(self, cutoff, atoms):
        self.cutoff = cutoff

        atoms.numbers = [1 if x == 0 else x for x in atoms.numbers]

        self.struct = AseAtomsAdaptor.get_structure(atoms)
        
    def get_neighbors(self, i, cutoff = None):
        if cutoff == None:
            cutoff = self.cutoff
        assert (
            cutoff == self.cutoff
        ...
        center_indices, points_indices, offset_vectors, distances = self.struct.get_neighbor_list(r=self.cutoff,
                                              sites = [self.struct.sites[i]],
                                              numerical_tol=0,
                                              exclude_self=True
                                             )
        
        return points_indices, offset_vectors, distances**2
...
def atoms_to_graph(atoms, cutoff):
    ...
    if np.any(atoms.get_cell().lengths() <= 0.0001) or (np.any(atoms.get_pbc()) and np.any(atoms.get_cell().lengths() < cutoff)):
        ...
        #neighborlist = asap3.FullNeighborList(cutoff, atoms)
        #neighborlist = AseNeigborListWrapper(cutoff, atoms)
        neighborlist = PymatgenNeighborListWrapper(cutoff, atoms)
        #print('Warning: DeepDFT wanted to use asap3 here')
...
def probes_to_graph(atoms, probe_pos, cutoff, neighborlist=None):
    ...
    if hasattr(neighborlist, "get_neighbors_querypoint"):
        ...
        if np.any(atoms.get_cell().lengths() <= 0.0001) or (np.any(atoms.get_pbc()) and np.any(atoms.get_cell().lengths() < cutoff)):
            ...
            #neighborlist = asap3.FullNeighborList(cutoff, atoms_with_probes)
            #neighborlist = AseNeigborListWrapper(cutoff, atoms_with_probes)
            neighborlist = PymatgenNeighborListWrapper(cutoff, atoms_with_probes)
            #print('Warning: DeepDFT wanted to use asap3 here')
...
def _calculate_grid_pos(density, origin, cell):
    ...
    grid_pos = np.dot(grid_pos, cell.cpu())
